...
第三次调用：Generator函数从上次yield语句停下的地方，一直执行到return语句
            (如果没有return语句，就执行到函数结束).
            next方法返回的对象的value属性，就是紧跟在return语句后面的表达式的值
            (如果没有return语句，则value属性的值为undefined)，done属性的值true，表示遍历已经结束。
第四次调用：此时Generator函数已经运行完毕，
...
ES6没有规定：
          function关键字与函数名之间的星号，写在哪个位置。这导致下面的写法都能通过。
          function * foo(x, y) { ··· }
          function *foo(x, y) { ··· }
          function* foo(x, y) { ··· }
          function*foo(x, y) { ··· }
...
/*1.2.2、Generator函数可以不用yield语句，这时就变成了一个单纯的暂缓执行函数。*/
function* f() {
  console.log('执行了！');
}
var generator = f();
...
/*1.2.3、另外需要注意，yield语句只能用在 Generator 函数里面，用在其他地方都会报错。*/
(function (){
  yield 1;
})()
...
/* 
上面代码也会产生句法错误，
因为forEach方法的参数是一个普通函数，
但是在里面使用了yield语句（这个函数里面还使用了yield*语句）。
一种修改方法是改用for循环。
*/
var arr = [1, [[2, 3], 4], [5, 6]];
var flat = function* (a) {
...
/*1.2.4、另外，yield语句如果用在一个表达式之中，必须放在圆括号里面。*/
function* demo() {
...
/*1.2.5、yield语句用作函数参数或放在赋值表达式的右边，可以不加括号。*/
function* demo() {
...
如果next方法没有参数，每次运行到yield语句，变量reset的值总是undefined。
...
导致y的值等于2 * undefined（即NaN），除以3以后还是NaN，
...
第三次调用next方法，将上一次yield语句的值设为13，因此z等于13，
这时x等于5，y等于24，所以return语句的值等于42。
*/
/* 
注意，由于next方法的参数表示上一个yield语句的返回值，
所以第一次使用next方法时，不能带有参数。
...
// First input: hello!
/* 
上面代码中，Generator函数如果不用wrapper先包一层，
是无法第一次调用next方法，就输入参数的。
...
/*3.3、扩展运算符+Array.from+解构赋值*/
/* 
除了for...of循环以外，扩展运算符（...）、
解构赋值和Array.from方法内部调用的，都是遍历器接口。
这意味着，它们都可以将Generator函数返回的Iterator对象，作为参数。
*/
function* numbers () {
  yield 1
  yield 2
  return 3 //遇到return即为true，就会返回，但是不包含return
  yield 4
}
// 扩展运算符
[...numbers()] // [1, 2]
...
*/
/*4.1、i.throw()-->捕获内部catch，外部catch-->捕获外部new Error()*/
var g = function* () {
  try {
    yield;//yield没有value，所以报错
  } catch (e) {
    console.log('内部捕获', e);
  }
};
...
/*4.2、i.throw()-->【传参new Error()捕获外部异常】*/
...
/* 
上面代码之所以只捕获了a，
是因为函数体外的catch语句块，捕获了抛出的a错误以后，
就不会再继续try代码块里面剩余的语句了。
*/

/*4.4、i.throw()-->捕获内部catch【内部无catch，直接走外部catch】*/
/* 
...
/*4.5、g.throw()-->捕获内部catch【内部无catch，外部无catch，中断执行】*/
/* 
如果Generator函数内部和外部，都没有部署try...catch代码块，
那么程序将报错，直接中断执行。
*/
...
/* 
上面代码中，g.throw抛出错误以后，
没有任何try...catch代码块可以捕获这个错误，导致程序报错，中断执行。
*/

/*4.6、g.throw()-->捕获内部catch【继续执行一次next方法】*/
/* 
...
g.next() // c
/* 
上面代码中，g.throw方法被捕获以后，
自动执行了一次next方法，所以会打印b。
另外，也可以看到，只要Generator函数内部部署了try...catch代码块，
那么遍历器的throw方法抛出的错误，不影响下一次遍历。
*/

/*4.7、外部catch捕获-->new Error()【new Error()不影响next()】*/
...
/* 
上面代码中，第二个next方法向函数体内传入一个参数42，
数值是没有toUpperCase方法的，
所以会抛出一个TypeError错误，被函数体外的catch捕获。
*/

/*4.9、外部catch没有捕获到-->内部非catch【语法错误】*/
/* 
...
function log(generator) {
  ...
  } catch (err) {//内部throw new Error()所以到catch
    console.log('捕捉错误', v);// 和第一次一样：捕捉错误 { value: 1, done: false }
  }
...
然后第三次运行的时候，Generator函数就已经结束了，不再执行下去了
*/
...
结果就是，outer1返回一个遍历器对象，
outer2返回该遍历器对象的内部值。

从语法角度看，如果yield命令后面跟的是一个遍历器对象，
需要在yield命令后面加上星号，表明它返回的是一个遍历器对象。
这被称为yield*语句。
*/
...
/*6.2、yield*遍历无return的Generator函数*/
/* 
...
/*6.4、yield*遍历有return的Generator函数*/
/* 
如果被代理的Generator函数有return语句，
...
/* 
上面代码中，myGeneratorMethod属性前面有一个星号，
...
ES6规定这个遍历器是Generator函数的实例，
也继承了Generator函数的prototype对象上的方法。
*/
/*8.1、Generator函数实例不用new*/
function* g() {}
...
obj.a // undefined
/* 
上面代码中，Generator函数g在this对象上面添加了一个属性a，
但是obj对象拿不到这个属性。
*/

/*8.2、Generator函数用new报错*/
function* F() {
  yield this.x = 2;
  yield this.y = 3;
}
...
那么，有没有办法让Generator函数返回一个正常的对象实例，
...
/* 
Generator是实现状态机的最佳结构。比如，上面的clock函数就是一个状态机。
...
它们的不同之处在于，同一时间可以有多个线程处于运行状态，
但是运行的协程只能有一个，其他协程都处于暂停状态。
此外，普通的线程是抢先式的，到底哪个线程优先得到资源，必须由运行环境决定，
...
如果将Generator函数当作协程，完全可以将多个需要互相协作的任务写成Generator函数，
它们之间使用yield语句交换控制权。
*/
...
/*10.3、部署Iterator接口*/
/* 
下面代码中，myObj是一个普通对象，通过iterEntries函数，就有了Iterator接口。
...
/* 
上面代码就是依次返回三个函数，但是由于使用了Generator函数，
