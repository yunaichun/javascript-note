...
另一种是利用Array.from方法。
下面代码提供了两种方法，直接在遍历操作中改变原来的Set结构。
*/
// 方法一
let set = new Set([1, 2, 3]);
...
2、其次，WeakSet中的对象都是弱引用，即垃圾回收机制不考虑WeakSet对该对象的引用，
   也就是说，如果其他对象都不再引用该对象，
...
*/
/* 
注意，是a数组的成员成为WeakSet的成员，而不是a数组本身。
这意味着，数组的成员只能是对象。
下面代码中，数组b的成员不是对象，加入WeaKSet就会报错。
*/
var b = [3, 4];
...
这里使用WeakSet的好处是，foos对实例的引用，不会被计入内存回收机制，
...
该数组的成员是一个个表示键值对的数组。
下面代码在新建Map实例时，就指定了两个键name和title。
*/
var map = new Map([
  ['name', '张三'],
  ['title', 'Author']
...
/* 
注意，只有对同一个对象的引用，
Map结构才将其视为同一个键。这一点要非常小心。
*/
var map = new Map();
map.set(['a'], 555);///////////////////////////////////////////对象引用不一样
map.get(['a']) // undefined
/* 
...
/* 
上面代码最后的那个例子，
表示Map结构的默认遍历器接口（Symbol.iterator属性），
就是entries方法。
*/
...
下面代码中，forEach方法的回调函数的this，就指向reporter。
*/
var reporter = {
...
/* 
WeakMap结构与Map结构基本类似，
唯一的区别是它只接受对象作为键名（null除外），
不接受其他类型的值作为键名，而且键名所指向的对象，不计入垃圾回收机制。
下面代码中，如果将1和Symbol作为WeakMap的键名，都会报错。
*/
var map = new WeakMap()
map.set(1, 2)
...
/* 
上面代码中，
变量wm是一个WeakMap实例，我们将一个DOM节点element作为键名，然后销毁这个节点，
